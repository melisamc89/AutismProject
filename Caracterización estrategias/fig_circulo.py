...
def calculo_b_circulo(name,cant,directorio1,variable):
    ...
    b=0
    for i in range(10):
        b= b +(2*presentado[i]-3*math.pi)*(theta[i]-3*math.pi+presentado[i])
    return b

def calculo_a_circulo():
    ...
    a=0
    for i in range(10):
        a = a+ pow(2*pres[i]-3*math.pi,2)
    return a

def t_chi2_circulo(directorio1,directorio2,name,cant,edad,sexo,pob,alfa,media,variable):
    ...
    if variable ==0:
        file = open(os.path.join('.',directorio2,'chi2_circulo.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_circulo.txt'),'a')
    else:
        file = open(os.path.join('.',directorio2,'chi2_circulo_pereza.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_circulo_pereza.txt'),'a')
    ...
    var_prom =[]
    for i in range (10):
        var_prom.append((2*angulo[i] - 3*math.pi)/2)
    
    chi2=0
    m=0
    
    b = calculo_b_circulo(name,cant,directorio1,variable)
    a = calculo_a_circulo()
    p= b/a
    #print(name)
    #print (str(p))
    ...
    for i in range(10):
        chi2 = chi2 + pow((media[i]-p*(2*angulo[i]-3*math.pi)-(3*math.pi-angulo[i]))/var_prom[i],2)
    #print(str(chi2))
