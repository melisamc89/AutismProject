...
def calculo_b_estrella(name,cant,directorio1,variable):
    ...
    b=0
    for i in range(10):
        b= b +(2*presentado[i]-2*math.pi)*(theta[i]-2*math.pi+presentado[i])
    return b

def calculo_a_estrella():
    ...
    a=0
    for i in range(10):
        a = a+ pow(2*pres[i]-2*math.pi,2)
    return a

def t_chi2_estrella(directorio1,directorio2,name,cant,edad,sexo,pob,alfa,media,variable):
    ...
    if variable ==0:
        file = open(os.path.join('.',directorio2,'chi2_estrella.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_estrella.txt'),'a')
    else:
        file = open(os.path.join('.',directorio2,'chi2_estrella_pereza.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_estrella_pereza.txt'),'a')
    ...
    var_prom =[]
    for i in range (10):
        var_prom.append(angulo[i] - math.pi)
    
    chi2=0
    m=0
    
    b = calculo_b_estrella(name,cant,directorio1,variable)
    a = calculo_a_estrella()
    p= b/a
    #print(name)
    #print (str(p))
