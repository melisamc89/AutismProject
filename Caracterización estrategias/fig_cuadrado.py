...
def calculo_b_cuadrado(name,cant,directorio1,variable):
...
def t_chi2_cuadrado(directorio1,directorio2,name,cant,edad,sexo,pob,alfa,media,variable):
    ...
    if variable ==0:
        file = open(os.path.join('.',directorio2,'chi2_cuadrado.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_cuadra.txt'),'a')
    else:
        file = open(os.path.join('.',directorio2,'chi2_cuadrado_pereza.txt'),'a')
        archivo = open(os.path.join('.',directorio2,'aceptados_cuadrado_pereza.txt'),'a')
    ...
    var_prom =[]
    for i in range (10):
        var_prom.append((2*angulo[i] - math.pi)/2)
    
    chi2=0
    m=0
    
    b = calculo_b_cuadrado(name,cant,directorio1,variable)
    a = calculo_a_cuadrado()
    p= b/a
    #print(name)
    #print (str(p))
    ...
    for i in range(10):
        chi2 = chi2 + pow((media[i]-p*(2*angulo[i]-math.pi)-(math.pi-angulo[i]))/var_prom[i],2)
    #print(str(chi2))
